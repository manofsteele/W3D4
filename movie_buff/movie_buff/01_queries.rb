...
def harrison_ford
  ...
  .where(castings: {ord: 1}).where(actors: {name: 'harrison Ford'})
  Movie.joins(:actors).where("name = ? AND ord > ?", 'Harrison Ford', 1).select('movies.id, movies.title') # castings ord is not 
end
